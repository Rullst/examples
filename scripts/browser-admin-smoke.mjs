...
async function startLoopbackProxy() {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (input.session) { headers.set('cookie', `${headers.get('cookie') || ''}; rullst_session=${input.session}`); } else { headers.set('authorization', authorization); }
      const chunks = [];
...
async function run() {
  ...
  for (const [panel, page] of [['nexus', 'chat'], ['studio', 'ai']]) {
    ...
    await evaluate(`(() => {
      ...
      const cloud = document.querySelector('[name=cloud_ai]'); if (cloud) cloud.checked = true;
      form.requestSubmit();
      return true;
    })()`);

...
pub fn owner_certificate_page(
    ...
    let body = format!(
        r#"<main class="page"><nav class="actions" aria-label="Certificate actions"><a href="/dashboard">Back to dashboard</a><button id="print-certificate" type="button">Print or save as PDF</button></nav><article class="certificate"><div class="mark" aria-hidden="true">R</div><p class="eyebrow">Rullst verified sandbox participation</p><h1>{badge}</h1><p class="awarded">Awarded to</p><p class="holder">{holder}</p><p class="description">For completing a provider-confirmed Stripe sandbox purchase through the Rullst SaaS staging blueprint. No real-money charge is represented by this certificate.</p><dl><div><dt>Verification ID</dt><dd>{public_id}</dd></div><div><dt>Issued</dt><dd>{issued}</dd></div><div><dt>Environment</dt><dd>{environment}</dd></div><div><dt>Status</dt><dd class="state state--{state_class}">{state}</dd></div></dl><p class="verify">Verify without exposing personal data at <a href="/verify/{public_id}">/verify/{public_id}</a></p></article><p class="privacy">Your name appears only on this authenticated private page. The public verification page contains no name, email, payment identifier or financial details.</p></main>"#,
...
pub fn public_verification_page(
    ...
    let body = format!(
        r#"<main class="page page--verify"><article class="verification"><div class="mark" aria-hidden="true">R</div><p class="eyebrow">Public certificate verification</p><h1>{badge}</h1><p class="verification-result state state--{state_class}">{state}</p><dl><div><dt>Verification ID</dt><dd>{public_id}</dd></div><div><dt>Issued</dt><dd>{issued}</dd></div><div><dt>Environment</dt><dd>{environment}</dd></div></dl><div class="sandbox-warning"><strong>Sandbox certificate</strong><p>This confirms a Stripe test-mode transaction. It does not prove a real-money purchase or customer status.</p></div><p class="privacy">Privacy by default: this page does not disclose the holder's name, email, provider IDs or payment details.</p><a class="home-link" href="/">Rullst SaaS blueprint</a></article></main>"#,
        badge = rullst::html::escape_str(badge_label(badge_kind)),

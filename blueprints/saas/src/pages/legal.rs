...
pub fn production_privacy_notice_page(merchant: &MerchantNotice, csp_nonce: &str) -> Html<String> {
    ...
    let content = format!(
        ...
        <section><h2>Data processed</h2><p>The service stores the account name, normalized email, Argon2id password hash, internal identifiers, purchase attempts, entitlement state, certificate state, refund-request state and bounded security metadata. Password recovery additionally uses hashed short-lived reset codes, keyed request-limit identifiers, delivery state and revocable session references; complete reset codes are not stored. Stripe hosts payment collection; Rullst does not receive or store complete card numbers or security codes.</p></section>
        <section><h2>Purposes and providers</h2><p>Data is processed to create, authenticate and recover accounts, prevent abuse, complete the requested one-time purchase, reconcile signed Stripe events, deliver the purchased artifact, issue and verify a certificate, handle refunds and disputes, maintain security and meet legal obligations. The application runs in Microsoft Azure, application records are stored in Neon PostgreSQL, payments are processed by Stripe, and enabled password-recovery messages are delivered by Resend using the account email and deterministic security content without marketing or open/click tracking. These providers can process data internationally under their respective contractual safeguards.</p></section>

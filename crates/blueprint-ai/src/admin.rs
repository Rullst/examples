...
async fn query(State(state): State<AdminAi>, request: Request) -> Response {
    ...
    if input.cloud_ai.as_deref() != Some("yes") {
        return private(Html(render_markdown(&format!(
            "**Local guidance · cloud AI is off**\n\n{}\n\nNo message was sent to an external AI provider. Enable the optional cloud choice for a generated answer. No action was executed.", state.blueprint.context()
        ))).into_response());
    }

...
#[tokio::test]
async fn cloud_is_off_without_explicit_choice() {
    let response = app(Blueprint::Lms, Surface::Nexus)
        .oneshot(request(
            "/nexus/copilot/query",
            Method::POST,
            true,
            ...
            true,
            "message=Explain+this+panel&_token=test-csrf-token",
        ))
        .await
        .unwrap();
    ...
    assert!(html.contains("No message was sent to an external AI provider"));
}
